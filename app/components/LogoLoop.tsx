  ...
  const renderLogoItem = useCallback((item: LogoItem, key: React.Key) => {
    ...
    const content = isNode
      ...
      : <img src={(item as ImageLogoItem).src} srcSet={(item as ImageLogoItem).srcSet} sizes={(item as ImageLogoItem).sizes}
          ...
          alt={(item as ImageLogoItem).alt ?? ""} title={item.title} loading="lazy" decoding="async" draggable={false} />;
    const label = isNode ? ((item as NodeLogoItem).ariaLabel ?? item.title) : ((item as ImageLogoItem).alt ?? item.title);
